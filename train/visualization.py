...
def dimension_reduction(repres_list, label_list, epoch,config):
    ...
    # util_dim_reduction.t_sne(title, repres_list, label_list, None, 2,config)
    # print('PCA')
    # title = 'Samples Embedding PCA Visualisation, Epoch[{}]'.format(epoch)
    # util_dim_reduction.pca(title, repres_list, label_list, None, 4)
    print('UMAP')
    title = 'Samples Embedding UMAP Visualisation, Epoch[{}]'.format(epoch)
    util_dim_reduction.Umap(title, repres_list, label_list, None, 2,config,epoch)
...
def penultimate_feature_visulization(repres_list, label_list, epoch,config):
    ...
    plt.savefig(config.result_folder + '/' + 'Feature Visualization' + '.png')
    plt.show()
